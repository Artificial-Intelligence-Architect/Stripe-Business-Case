-- ============================================================
-- SQL/OLTP: Modèle transactionnel Stripe (PostgreSQL)
-- ============================================================
...
CREATE TABLE customers (
    ...
    email_hash      CHAR(64),            -- SHA-256 (pour analytics sans PII)
    country_code    CHAR(2)      REFERENCES countries(code),
    created_at      TIMESTAMPTZ  DEFAULT now()
);

-- 3. TABLE DE FAITS TRANSACTIONS (partitionnée)
CREATE TABLE transactions (
    ...
    amount_usd      NUMERIC(18,4),       -- converti par trigger (non détaillé ici)
    ...
    PRIMARY KEY (transaction_id, created_at)  -- clé composite pour partitionnement
) PARTITION BY RANGE (created_at);

-- Création manuelle des partitions mensuelles (exemple pour janvier 2024)
CREATE TABLE transactions_2024_01 PARTITION OF transactions
    FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');
-- (en production, automatiser avec pg_partman ou Cron)

-- 4. INDEX STRATÉGIQUES
-- Requêtes par merchant
CREATE INDEX idx_txn_merchant_date ON transactions (merchant_id, created_at DESC);
-- Requêtes par customer
CREATE INDEX idx_txn_customer_date ON transactions (customer_id, created_at DESC);
-- Monitoring des statuts en attente/échec
CREATE INDEX idx_txn_status_pending ON transactions (status, created_at DESC)
    WHERE status IN ('pending', 'failed');
-- Détection fraude : uniquement les scores élevés (index partiel)
...
-- Trigger d'audit générique pour la table transactions
...
-- Application du trigger sur la table transactions
