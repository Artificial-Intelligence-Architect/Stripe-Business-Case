-- ============================================================
-- SQL/OLAP: Analytical Queries (Snowflake-compatible)
-- ============================================================

-- 1. Monthly revenue per merchant with MoM growth
...
SELECT
    *,
    LAG(revenue_usd) OVER (
        PARTITION BY merchant_name ORDER BY year, month
    ) AS prev_month_revenue,
    ROUND(
        (revenue_usd - LAG(revenue_usd) OVER (
            PARTITION BY merchant_name ORDER BY year, month)
        ) * 100.0 / NULLIF(LAG(revenue_usd) OVER (
            PARTITION BY merchant_name ORDER BY year, month), 0),
        2
    ) AS mom_growth_pct
...
SELECT
    ...
    ROUND(
        SUM(IFF(f.is_fraud, 1, 0)) * 100.0 / COUNT(*), 3
    )                                               AS fraud_rate_pct,
